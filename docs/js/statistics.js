/* ════════════════════════════════════════════════════
  ...
  (function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      try {
        var authStr = localStorage.getItem(('sb-' + (window._SB_CONFIG?.url?.match(/https:\/\/([^.]+)\.supabase\.co/)?.[1] || 'wsorngsyowgxikiepice') + '-auth-token')) || '';
    ...
    // ── Setup member selector for managers ──
    function setupMemberSelector() {
      ...
      picker.innerHTML = '<option value="__all">📊 ทุกคนรวม</option><option value="__self">👤 ของฉัน</option>';
      bandMembers.forEach(function(m) {
        var o = document.createElement('option');
        o.value = m.memberId || m.id;
        o.textContent = '👤 ' + (m.name || 'ไม่ระบุชื่อ');
    ...
    function getDateRange() {
      ...
      for (var dd = new Date(start); dd <= end; dd.setDate(dd.getDate() + 1)) {
        var _dd = new Date(dd); dates.push(_dd.getFullYear() + '-' + String(_dd.getMonth()+1).padStart(2,'0') + '-' + String(_dd.getDate()).padStart(2,'0'));
      }
    ...
    function loadAll() {
      ...
      // Show loading
      document.getElementById('overviewCards').innerHTML = '<div class="stat-card"><div class="ico">⏳</div><div class="val">กำลังโหลด...</div></div>';
      document.getElementById('detailTbody').innerHTML = '<tr><td colspan="7" style="text-align:center;color:var(--premium-text-muted)">กำลังโหลด...</td></tr>';
      ...
      var reqId = ++_loadAllReqId;
      var ciDone = false, lvDone = false;
      var isManager = (myRole === 'admin' || myRole === 'manager');
      // สมาชิกธรรมดา: fetch เฉพาะของตัวเอง | ผู้จัดการ: fetch ทั้งวง
      var memberFilter = isManager ? undefined : myId;

      // Single range query for check-ins
      apiCall('getCheckInsForRange', { bandId: bandId, dateFrom: dateFrom, dateTo: dateTo, memberId: memberFilter }, function(r) {
        if (reqId !== _loadAllReqId) return;
      ...
      apiCall('getLeaveRequestsForRange', { bandId: bandId, dateFrom: dateFrom, dateTo: dateTo, memberId: memberFilter }, function(r) {
        if (reqId !== _loadAllReqId) return;
    ...
    function renderStats() {
      ...
      if (sel === '__all' && isManager) {
        ...
        if (!stats) {
          document.getElementById('overviewCards').innerHTML = '<div class="stat-card"><div class="ico">📭</div><div class="val">ไม่พบข้อมูล</div><div class="lbl">ยังไม่มีข้อมูลลงเวลาในช่วงนี้</div></div>';
          document.getElementById('detailTbody').innerHTML = '<tr><td colspan="7" style="text-align:center;color:var(--premium-text-muted)">ไม่พบข้อมูล</td></tr>';
          destroyCharts();
          return;
        }
    ...
    function renderSingleMember(s, mid) {
      var cardsHtml = ''
        + card('⏱️', s.totalBreaks + ' เบรค', 'เบรคทำงาน (' + s.totalHours.toFixed(1) + ' ชม.)', '')
        + card('💰', formatCurrency(s.totalEarnings) + ' ฿', 'รายได้รวม', 'green')
        + card('🚫', (s.leaveDaysWithSub + s.leaveDaysNoSub) + ' วัน', 'ลา (มีแทน ' + s.leaveDaysWithSub + ' / ไม่มี ' + s.leaveDaysNoSub + ')', 'purple')
        + card('🔄', formatCurrency(s.totalSubCost) + ' ฿', 'จ่ายคนแทนรวม', 'red')
        + card('💵', formatCurrency(s.netEarnings) + ' ฿', 'คงเหลือหลังหักคนแทน', 'blue');
    ...
    function renderAllMembersView() {
      ...
      bandMembers.forEach(function(m) {
        ...
        if (!s) return;
        allStats.push({ name: m.name || 'ไม่ระบุ', mid: mid, stats: s });
      ...
      var cardsHtml = ''
        + card('👥', bandMembers.length + ' คน', 'สมาชิกทั้งหมด', '')
        + card('⏱️', totB + ' เบรค', 'เบรครวม (' + totH.toFixed(1) + ' ชม.)', '')
        + card('💰', formatCurrency(totE) + ' ฿', 'รายได้รวมทั้งวง', 'green')
        + card('🚫', (totLW + totLN) + ' วัน', 'ลารวม (แทน ' + totLW + ' / ไม่มี ' + totLN + ')', 'purple')
        + card('🔄', formatCurrency(totSC) + ' ฿', 'จ่ายคนแทนทั้งวง', 'red');
      document.getElementById('overviewCards').innerHTML = cardsHtml;

      var tbody = document.getElementById('allMembersTbody');
      if (!allStats.length) {
        tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;color:var(--premium-text-muted)">ไม่พบข้อมูล</td></tr>';
      } else {
    ...
    function renderDetailTable(s) {
      ...
      if (!s.daily.length) {
        tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:var(--premium-text-muted)">ไม่พบข้อมูลในช่วงนี้</td></tr>';
        return;
      }
      tbody.innerHTML = s.daily.map(function(d) {
        var dt = new Date(d.date);
        var dateLabel = DN_SHORT[d.dow] + ' ' + dt.getDate() + '/' + (dt.getMonth()+1);
        var status = d.isLeave ? (d.hasSub ? '<span class="has-sub">🚫 ลา (มีแทน)</span>' : '<span class="no-sub">🚫 ลา (ไม่มีแทน)</span>') : '✅ เข้างาน';
        var subInfo = d.subName ? '🔄 ' + escapeHtml(d.subName) : '-';
      ...
      + '<tr style="background:#ebf8ff;font-weight:700">'
        + '<td colspan="4">💵 คงเหลือหลังหักคนแทน</td>'
        + '<td colspan="3" style="text-align:right;color:' + (s.netEarnings >= 0 ? '#276749' : '#e53e3e') + ';font-size:var(--text-base)">' + formatCurrency(s.netEarnings) + ' ฿</td></tr>';
    }
    ...
    function renderCharts(s) {
      ...
      chartLeaveInst = new Chart(document.getElementById('chartLeave'), {
        type: 'doughnut',
        data: {
          labels: ['เข้างาน', 'ลา (มีแทน)', 'ลา (ไม่มีแทน)'],
          datasets: [{
      ...
      if (s.subDeductions.length) {
        ...
        ctx.fillText('ไม่มีข้อมูลคนแทน', cvs.width / 2, 60);
      }
    }
    ...
    function renderChartsAllMembers(allStats) {
      ...
      chartEarningsInst = new Chart(document.getElementById('chartEarnings'), {
        type: 'bar',
        data: { labels: names, datasets: [{ label: 'รายได้ (฿)', data: allStats.map(function(ms) { return ms.stats.totalEarnings; }), backgroundColor: '#c6f6d5', borderRadius: 4 }] },
        options: chartOpts('฿')
      });

      chartHoursInst = new Chart(document.getElementById('chartHours'), {
        type: 'bar',
        data: { labels: names, datasets: [{ label: 'ชั่วโมง', data: allStats.map(function(ms) { return Math.round(ms.stats.totalHours * 10) / 10; }), backgroundColor: '#bee3f8', borderRadius: 4 }] },
        options: chartOpts('ชม.')
      });
      ...
      chartLeaveInst = new Chart(document.getElementById('chartLeave'), {
        ...
        data: {
          labels: names,
          datasets: [
            { label: 'ลา (มีแทน)', data: allStats.map(function(ms) { return ms.stats.leaveDaysWithSub; }), backgroundColor: '#d6bcfa', borderRadius: 4 },
            { label: 'ลา (ไม่มีแทน)', data: allStats.map(function(ms) { return ms.stats.leaveDaysNoSub; }), backgroundColor: '#fed7d7', borderRadius: 4 }
          ]
        },
        options: Object.assign({}, chartOpts('วัน'), { plugins: { legend: { display: true, position: 'bottom', labels: { font: { family: 'Sarabun', size: 11 } } } }, scales: { x: { stacked: true, ticks: { font: { family: 'Sarabun', size: 11 } } }, y: { stacked: true, beginAtZero: true, ticks: { font: { family: 'Sarabun', size: 11 } } } } })
      });

      chartSubInst = new Chart(document.getElementById('chartSub'), {
        type: 'bar',
        data: { labels: names, datasets: [{ label: 'จ่ายคนแทน (฿)', data: allStats.map(function(ms) { return ms.stats.totalSubCost; }), backgroundColor: '#e9d8fd', borderRadius: 4 }] },
        options: chartOpts('฿')
      });
    }
